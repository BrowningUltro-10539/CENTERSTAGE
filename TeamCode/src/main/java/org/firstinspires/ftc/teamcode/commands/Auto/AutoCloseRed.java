...
public class AutoCloseRed extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        TrajectorySequence toRightTape = robot.driveSubsystem.trajectorySequenceBuilder(startPose)
                ...
                .lineTo(new Vector2d(31, -30))
                .build();
        TrajectorySequence toCenterRFBackdrop = robot.driveSubsystem.trajectorySequenceBuilder(toRightTape.end())
                .lineTo(new Vector2d(51.5, -40.5))
                .build();

        //Left tape
        TrajectorySequence toLeftTape = robot.driveSubsystem.trajectorySequenceBuilder(startPose)
                .lineTo(new Vector2d(12, -30.5))
                .turn(Math.toRadians(-90))
                .lineTo(new Vector2d(8, -30.5))
                .build();
        TrajectorySequence toCenterLFBackdrop = robot.driveSubsystem.trajectorySequenceBuilder(toLeftTape.end())
                .lineTo(new Vector2d(52, -31.5))
                .build();
            ...
            case LEFT:
                CommandScheduler.getInstance().schedule(
                        new SequentialCommandGroup(
                                ...
                                new IntakeRunCommand(robot.intake, -0.40, .35),
                                new TrajectorySequenceFollowerCommand(robot.driveSubsystem, toCenterLFBackdrop),
                                new LiftPositionCommand(robot.lift, 10, 200, 200, 2),
                                new InstantCommand(() -> robot.outtake.update(OuttakeSubsystem.ArmState.RELEASE)),
                                new WaitCommand(2000),
                                new DepositAndRetractCommand(robot)
            ...
            case RIGHT:
                CommandScheduler.getInstance().schedule(
                        new SequentialCommandGroup(
                                ...
                                new IntakeRunCommand(robot.intake, -0.40, .35),
                                new TrajectorySequenceFollowerCommand(robot.driveSubsystem, toCenterRFBackdrop),
                                new LiftPositionCommand(robot.lift, 6, 200, 200, 2),
                                new InstantCommand(() -> robot.outtake.update(OuttakeSubsystem.ArmState.RELEASE)),
                                new WaitCommand(2000),
                                new DepositAndRetractCommand(robot)
                        )
